...
class DiscordClient(MltemplateBase):
    ...
    def discord_bot(self):
        ...
        async def load_model(
            ...
            self.logger.debug(f'Received load_model request from user {ctx.author} with arguments: model={model}, '
        ...
        async def logs(ctx):
            ...
            try:
                await ctx.send(file=discord.File(os.path.join(Config()['DIR_PATHS']['LOGS'], 'gateway_server_logs.txt')))
            except Exception as err:
                self.logger.exception(f'Error raised in sending gateway server logs:\n{err}')
            try:
                await ctx.send(file=discord.File(os.path.join(Config()['DIR_PATHS']['LOGS'], 'training_server_logs.txt')))
            except Exception as err:
